...
var voidlatency_core_default = {
  async fetch(request, env, ctx) {
    ...
    const isApi = url.pathname.startsWith("/api/") || url.pathname === "/locations";

    // GUARD: if D1 isn't bound, never throw 1101 - show a clear message
    if (!env || !env.VL_DB) {
      if (isApi) {
        return new Response(JSON.stringify({ error: "D1 database not bound. In the Worker → Settings → Bindings, add a D1 binding named exactly VL_DB." }), { status: 503, headers: { "Content-Type": "application/json" } });
      }
      return new Response(
        "<html><body style='font-family:sans-serif;background:#0d1b2a;color:#e6ecf5;display:flex;align-items:center;justify-content:center;height:100vh;text-align:center'><div><h2>⚠️ Database not connected</h2><p>Add a <b>D1 binding named VL_DB</b> to this Worker (Settings → Bindings), then reload.</p></div></body></html>",
        { status: 503, headers: { "Content-Type": "text/html; charset=utf-8" } }
      );
    }

    try {
      try { await DbService.ensureSchema(env.VL_DB); } catch (e) {}
      try { await loadAdmins(env); } catch (e) {}
      ...
      // Never a raw 1101 - return the real error so it's debuggable
      const msg = (e && e.message) ? e.message : String(e);
      if (isApi) {
        return new Response(JSON.stringify({ error: "Server error: " + msg }), { status: 500, headers: { "Content-Type": "application/json" } });
      }
      return new Response("Server error: " + msg, { status: 500, headers: { "Content-Type": "text/plain; charset=utf-8" } });
    }
  }
};
...
async function loadAdmins(env) {
  try {
    ...
    try {
      ...
      `).run();
    } catch (e2) { /* D1 missing/unavailable - never throw */ }
  }
}
...
var Router = {
  ...
  async handleApi(request, url, env, ctx) {
    ...
    // ============================================
    // NODES - register relay/external nodes (x-ui style)
    // ============================================
    if (url.pathname === "/api/nodes") {
      if (!authorized) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 });
      if (request.method === "GET") {
        try {
          const { results } = await env.VL_DB.prepare("SELECT * FROM nodes ORDER BY id DESC").all();
      ...
      if (request.method === "POST") {
        try {
          ...
          await env.VL_DB.prepare("INSERT INTO nodes (name, address, port, path, network, security, remark, enable) VALUES (?,?,?,?,?,?,?,?)").bind(
            b.name || "", b.address || "", parseInt(b.port) || 443, b.path || "/", b.network || "ws", b.security || "tls", b.remark || "", b.enable === 0 ? 0 : 1
          ).run();
          return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
        } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
      }
    ...
    if (url.pathname.startsWith("/api/nodes/")) {
      ...
      if (request.method === "PUT") {
        try {
          const b = await request.json();
          await env.VL_DB.prepare("UPDATE nodes SET name=?, address=?, port=?, path=?, network=?, security=?, remark=?, enable=? WHERE id=?").bind(
            b.name || "", b.address || "", parseInt(b.port) || 443, b.path || "/", b.network || "ws", b.security || "tls", b.remark || "", b.enable === 0 ? 0 : 1, id
          ).run();
          return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
        } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
      }
      if (request.method === "DELETE") {
        try { await env.VL_DB.prepare("DELETE FROM nodes WHERE id=?").bind(id).run(); return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } }); }
        catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
      }
    }

    // ============================================
    // NODE INFO - this worker as a node for other panels
    // ============================================
    if (url.pathname === "/api/node-info" && request.method === "GET") {
      if (!authorized) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 });
      const host = url.hostname;
      return new Response(JSON.stringify({
        success: true,
        ...
        host: host,
        sni: host,
        note: "Add this as a node/server in your other panel (x-ui / 3x-ui). It is a VLESS+WS+TLS endpoint on Cloudflare."
      }), { headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
    }

    // ============================================
    // UPDATE CONFIG - save self-update credentials (CF token etc.)
    // ============================================
    if (url.pathname === "/api/update-config") {
      ...
      if (request.method === "POST") {
        try {
          const b = await request.json();
          const pairs = [["cf_token", b.cf_token], ["cf_account", b.cf_account], ["cf_script", b.cf_script], ["gh_url", b.gh_url]];
          for (const [k, v] of pairs) { if (v !== void 0) await env.VL_DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)").bind(k, String(v)).run(); }
          return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
        } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
      }
      if (request.method === "GET") {
        const get = async (k) => { try { const r = await env.VL_DB.prepare("SELECT value FROM settings WHERE key=?").bind(k).first(); return r ? r.value : ""; } catch (e) { return ""; } };
        return new Response(JSON.stringify({
          cf_account: await get("cf_account"),
          cf_script: await get("cf_script"),
          gh_url: await get("gh_url") || "https://raw.githubusercontent.com/Void0Latency/panelv3/main/voidlatency-core.js",
          has_token: !!(await get("cf_token"))
        }), { headers: { "Content-Type": "application/json" } });
      }
    }

    // ============================================
    // SELF UPDATE - pull latest code from GitHub & redeploy this worker
    // ============================================
    if (url.pathname === "/api/self-update" && request.method === "POST") {
      if (!authorized) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 });
      try {
        const get = async (k) => { try { const r = await env.VL_DB.prepare("SELECT value FROM settings WHERE key=?").bind(k).first(); return r ? r.value : ""; } catch (e) { return ""; } };
        const token = await get("cf_token");
        const account = await get("cf_account");
        const script = await get("cf_script");
        let ghUrl = await get("gh_url") || "https://raw.githubusercontent.com/Void0Latency/panelv3/main/voidlatency-core.js";
        if (!token || !account || !script) {
          return new Response(JSON.stringify({ error: "Update not configured. Set CF Token, Account ID and Script name in Settings first." }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        const codeRes = await fetch(ghUrl + (ghUrl.includes("?") ? "&" : "?") + "t=" + Date.now());
        if (!codeRes.ok) return new Response(JSON.stringify({ error: "Could not fetch code from GitHub (" + codeRes.status + ")." }), { status: 502, headers: { "Content-Type": "application/json" } });
        const code = await codeRes.text();
        const metadata = { main_module: "voidlatency-core.js", compatibility_date: "2024-12-18", bindings: [{ type: "d1", name: "VL_DB", id: "KEEP" }] };
        // We must preserve existing bindings; use the simpler script-settings PUT with module.
        const form = new FormData();
        form.append("metadata", new Blob([JSON.stringify({ main_module: "voidlatency-core.js", compatibility_date: "2024-12-18", keep_bindings: ["d1_databases"] })], { type: "application/json" }));
        form.append("voidlatency-core.js", new Blob([code], { type: "application/javascript+module" }), "voidlatency-core.js");
        const put = await fetch("https://api.cloudflare.com/client/v4/accounts/" + account + "/workers/scripts/" + script, {
          method: "PUT", headers: { "Authorization": "Bearer " + token }, body: form
        });
        const pd = await put.json().catch(() => ({}));
        if (put.ok && pd.success) {
          return new Response(JSON.stringify({ success: true, message: "Panel code updated from GitHub. Database & users untouched." }), { headers: { "Content-Type": "application/json" } });
        }
        const err = (pd.errors && pd.errors[0] && pd.errors[0].message) ? pd.errors[0].message : "Update failed";
...
var DbService = {
  async ensureSchema(db) {
    ...
    try { await db.prepare("CREATE TABLE IF NOT EXISTS nodes (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, address TEXT, port INTEGER, path TEXT DEFAULT '/', network TEXT DEFAULT 'ws', security TEXT DEFAULT 'tls', remark TEXT, enable INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)").run(); } catch (e) {}
    schemaEnsured = true;
  },
...
<body>
    ...
    <div class="lg:ml-64 min-h-screen main-content">
        ...
        <main class="p-3 sm:p-6">
            ...
            <div id="page-nodes" class="page-section">
                <div class="glass rounded-2xl p-4 sm:p-6 max-w-2xl mb-4">
                    <h2 class="text-lg font-bold text-white mb-1">This Server as a Node</h2>
                    <p class="text-xs text-zinc-400 mb-3">Add this server to your other panel (x-ui / 3x-ui) as a VLESS + WS + TLS node.</p>
                    <div id="node-info-box" class="glass-light rounded-xl p-4 text-sm text-zinc-300 font-mono break-all">Loading...</div>
                    <button onclick="copyNodeInfo()" class="mt-3 w-full py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold rounded-xl transition text-sm">📋 Copy Node Info</button>
                </div>
                <div class="glass rounded-2xl p-4 sm:p-6 max-w-2xl">
                    <div class="flex items-center justify-between mb-4">
                        <div><h2 class="text-lg font-bold text-white">Relay Nodes</h2><p class="text-xs text-zinc-400">Register other servers as nodes</p></div>
                        <button onclick="openNodeModal()" class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold rounded-xl text-sm">+ Add Node</button>
                    </div>
                    <div id="nodes-list" class="space-y-2"><p class="text-zinc-400 text-sm">Loading...</p></div>
                </div>
                <!-- Node modal -->
                <div id="node-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background:rgba(0,0,0,0.7);backdrop-filter:blur(8px)">
                    <div class="glass rounded-3xl p-5 w-full max-w-md" style="background:#12121c">
                        <div class="flex items-center justify-between mb-4"><h3 class="text-lg font-bold text-white" id="node-modal-title">Add Node</h3><button onclick="closeNodeModal()" class="text-zinc-400">✕</button></div>
                        <input type="hidden" id="node-id">
                        <div class="space-y-3">
                            <input id="node-name" placeholder="Name (e.g. Germany-1)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                            <input id="node-address" placeholder="Address (e.g. de.example.com)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                            <input id="node-port" type="number" placeholder="Port (443)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                            <input id="node-path" placeholder="Path (/)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                            <input id="node-remark" placeholder="Remark (optional)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                            <div class="flex gap-2"><button onclick="closeNodeModal()" class="flex-1 py-3 bg-white/5 text-zinc-400 rounded-xl text-sm">Cancel</button><button onclick="saveNode()" class="flex-1 py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold rounded-xl text-sm">Save</button></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="page-settings" class="page-section">
                <div class="glass rounded-2xl p-4 sm:p-6 max-w-2xl mb-4">
                    <h2 class="text-lg font-bold text-white mb-3">🔥 Update Panel from GitHub</h2>
                    <p class="text-xs text-zinc-400 mb-3">When the code on GitHub changes, replace only the panel code (your database & users stay intact). Set these once:</p>
                    <div class="space-y-3">
                        <input id="upd-account" placeholder="Cloudflare Account ID" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                        <input id="upd-script" placeholder="Worker script name (e.g. fire-panel-xxxx)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                        <input id="upd-ghurl" placeholder="GitHub raw URL of voidlatency-core.js" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                        <input id="upd-token" type="password" placeholder="Cloudflare API Token (Workers Scripts: Edit)" class="w-full px-4 py-3 rounded-xl text-white text-sm outline-none">
                        <div class="flex gap-2">
                            <button onclick="saveUpdateConfig()" class="flex-1 py-3 bg-white/5 text-zinc-300 rounded-xl text-sm border border-zinc-700/50">Save Config</button>
                            <button onclick="runSelfUpdate()" id="self-update-btn" class="flex-1 py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold rounded-xl text-sm">🔥 Update Now</button>
                        </div>
                        <p id="upd-status" class="text-xs text-zinc-400"></p>
                    </div>
                </div>
                <div class="glass rounded-2xl p-4 sm:p-6 max-w-2xl">
    ...
    <script>
        ...
        // NODES + SELF UPDATE
        // ============================================
        var nodeInfoCache = null;
        async function loadNodeInfo() {
            try {
                var r = await fetch('/api/node-info'); var d = await r.json();
                nodeInfoCache = d;
                document.getElementById('node-info-box').innerHTML =
                    'Address: <b>' + d.address + '</b><br>Port: <b>' + d.port + '</b><br>Network: <b>' + d.network + '</b><br>Security: <b>' + d.security + '</b><br>Path: <b>' + d.path + '</b><br>SNI/Host: <b>' + d.host + '</b>';
            } catch (e) { document.getElementById('node-info-box').innerText = 'Error loading node info'; }
        }
        function copyNodeInfo() {
            if (!nodeInfoCache) return;
            var d = nodeInfoCache;
            var t = 'Address: ' + d.address + '\\nPort: ' + d.port + '\\nNetwork: ' + d.network + '\\nSecurity: ' + d.security + '\\nPath: ' + d.path + '\\nSNI/Host: ' + d.host;
            navigator.clipboard.writeText(t).then(function(){ alert('✅ Node info copied!'); }).catch(function(){ prompt('Copy:', t); });
        }
        async function loadNodesList() {
            ...
            try {
                ...
                if (!list.length) { el.innerHTML = '<p class="text-zinc-400 text-sm text-center py-4">No nodes yet.</p>'; return; }
                el.innerHTML = list.map(function(n) {
                    return '<div class="glass-light rounded-xl p-3 flex items-center gap-3"><div class="flex-1 min-w-0">' +
                        '<div class="text-white font-semibold text-sm">' + (n.name || '(no name)') + ' <span class="badge ' + (n.enable ? 'badge-success' : 'badge-danger') + '">' + (n.enable ? 'on' : 'off') + '</span></div>' +
                        '<div class="text-xs text-zinc-400">' + n.address + ':' + n.port + ' · ' + n.network + '/' + n.security + ' · ' + n.path + '</div>' +
                        '<div class="text-[11px] text-zinc-500">' + (n.remark || '') + '</div></div>' +
                        '<button onclick="editNode(' + n.id + ')" class="action-btn text-yellow-400">✏️</button>' +
                        '<button onclick="delNode(' + n.id + ')" class="action-btn text-red-400">🗑</button></div>';
                }).join('');
                window._nodes = list;
        ...
        function openNodeModal() {
            ...
            document.getElementById('node-port').value = '443';
            document.getElementById('node-path').value = '/';
            document.getElementById('node-remark').value = '';
            document.getElementById('node-modal-title').innerText = 'Add Node';
            var m = document.getElementById('node-modal'); m.classList.remove('hidden'); m.style.display = 'flex';
        }
        function closeNodeModal() { var m = document.getElementById('node-modal'); m.classList.add('hidden'); m.style.display = 'none'; }
        function editNode(id) {
            var n = (window._nodes || []).find(function(x){ return x.id === id; }); if (!n) return;
            document.getElementById('node-id').value = n.id;
            document.getElementById('node-name').value = n.name || '';
            document.getElementById('node-address').value = n.address || '';
            document.getElementById('node-port').value = n.port || 443;
            document.getElementById('node-path').value = n.path || '/';
            document.getElementById('node-remark').value = n.remark || '';
            document.getElementById('node-modal-title').innerText = 'Edit Node';
            var m = document.getElementById('node-modal'); m.classList.remove('hidden'); m.style.display = 'flex';
        }
        async function saveNode() {
            var id = document.getElementById('node-id').value;
            var body = { name: document.getElementById('node-name').value, address: document.getElementById('node-address').value, port: document.getElementById('node-port').value, path: document.getElementById('node-path').value, remark: document.getElementById('node-remark').value };
            if (!body.address) { alert('❌ Address required'); return; }
            try {
                ...
                if (r.ok) { closeNodeModal(); loadNodesList(); } else { var d = await r.json(); alert('❌ ' + (d.error || 'Failed')); }
            } catch (e) { alert('❌ Connection error'); }
        }
        async function delNode(id) { if (!confirm('Delete node?')) return; try { await fetch('/api/nodes/' + id, { method: 'DELETE' }); loadNodesList(); } catch (e) {} }

        async function loadUpdateConfig() {
            try {
                var r = await fetch('/api/update-config'); var d = await r.json();
                document.getElementById('upd-account').value = d.cf_account || '';
                document.getElementById('upd-script').value = d.cf_script || '';
                document.getElementById('upd-ghurl').value = d.gh_url || '';
                document.getElementById('upd-token').placeholder = d.has_token ? 'Token saved (leave blank to keep)' : 'Cloudflare API Token';
            } catch (e) {}
        }
        async function saveUpdateConfig() {
            var body = { cf_account: document.getElementById('upd-account').value, cf_script: document.getElementById('upd-script').value, gh_url: document.getElementById('upd-ghurl').value };
            var tok = document.getElementById('upd-token').value; if (tok) body.cf_token = tok;
            try {
                var r = await fetch('/api/update-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
                if (r.ok) { document.getElementById('upd-token').value = ''; alert('✅ Update config saved'); loadUpdateConfig(); } else alert('❌ Failed');
            } catch (e) { alert('❌ Connection error'); }
        }
        async function runSelfUpdate() {
            if (!confirm('Update panel code from GitHub now? Your database & users will NOT change.')) return;
            var btn = document.getElementById('self-update-btn'); var st = document.getElementById('upd-status');
            btn.disabled = true; btn.innerText = 'Updating...'; st.innerText = 'Fetching latest code & redeploying...'; st.style.color = '#60a5fa';
            try {
                var r = await fetch('/api/self-update', { method: 'POST' }); var d = await r.json();
                if (r.ok && d.success) { st.innerText = '✅ ' + d.message; st.style.color = '#34d399'; }
                else { st.innerText = '❌ ' + (d.error || 'Update failed'); st.style.color = '#ef4444'; }
            } catch (e) { st.innerText = '❌ ' + e.message; st.style.color = '#ef4444'; }
            finally { btn.disabled = false; btn.innerText = '🔥 Update Now'; }
        }
        ...
        function showPage(page) {
            ...
            var titles = {
                ...
                nodes: ['Nodes', 'Use this server as a node or add others'],
            ...
            if (page === 'nodes') { loadNodeInfo(); loadNodesList(); }
            if (page === 'settings') { loadUpdateConfig(); }
        ...
        function ensureLightStyle() {
            var s = document.getElementById('light-theme-style');
            if (!s) {
                s = document.createElement('style'); s.id = 'light-theme-style';
                s.textContent = [
                    'body.light-mode{background:#eef2f7 !important;color:#1e293b !important;}',
                    'body.light-mode .glass{background:#ffffff !important;border:1px solid #e2e8f0 !important;box-shadow:0 4px 16px rgba(15,23,42,.06) !important;}',
                    'body.light-mode .glass-light{background:#f5f8fc !important;border:1px solid #e2e8f0 !important;}',
                    'body.light-mode .sidebar{background:#ffffff !important;border-right:1px solid #e2e8f0 !important;}',
                    'body.light-mode .system-stat{background:#f8fafc !important;border:1px solid #e8edf3 !important;}',
                    'body.light-mode header{background:rgba(255,255,255,.9) !important;border-bottom:1px solid #e2e8f0 !important;}',
                    'body.light-mode .text-white{color:#0f172a !important;}',
                    'body.light-mode .text-zinc-400{color:#64748b !important;}',
                    'body.light-mode .text-zinc-300{color:#475569 !important;}',
                    'body.light-mode .text-zinc-500{color:#94a3b8 !important;}',
                    'body.light-mode .sidebar-link{color:#475569 !important;}',
                    'body.light-mode .sidebar-link:hover{background:#eef6f1 !important;color:#0f172a !important;}',
                    'body.light-mode .sidebar-link.active{background:rgba(16,185,129,.12) !important;color:#059669 !important;}',
                    'body.light-mode input,body.light-mode select,body.light-mode textarea{background:#f1f5f9 !important;border:1px solid #cbd5e1 !important;color:#0f172a !important;}',
                    'body.light-mode input::placeholder,body.light-mode textarea::placeholder{color:#94a3b8 !important;}',
                    'body.light-mode .border-zinc-800\\/30,body.light-mode .border-zinc-800\\/50{border-color:#e2e8f0 !important;}',
                    'body.light-mode table thead tr{color:#64748b !important;}',
                    'body.light-mode .bg-zinc-800{background:#e2e8f0 !important;}',
                    'body.light-mode .node-info-box,body.light-mode #node-info-box{color:#1e293b !important;}'
                ].join('\\n');
        ...
        function applyTheme(theme) {
            ...
            if (theme === 'light') {
                ...
                if (icon) icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M14 12a2 2 0 11-4 0 2 2 0 014 0z"/>';
