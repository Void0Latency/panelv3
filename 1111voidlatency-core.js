...
var voidlatency_core_default = {
  async fetch(request, env, ctx) {
    ...
    // Hard guard: if D1 isn't bound, say so clearly instead of throwing 1101
    if (!env || !env.VL_DB) {
      if (isApi) {
        return new Response(JSON.stringify({ error: "D1 database not bound (env.VL_DB missing). The deployer must create & bind the D1 database." }), { status: 503, headers: { "Content-Type": "application/json" } });
      }
    ...
    } catch (e) { /* swallow: schema/admins are best-effort */ }
    ...
    try {
      ...
      // NEVER throw a raw 1101 - surface the real error so we can see it
...
var DbService = {
  async ensureSchema(db) {
    ...
    // Only cache success if the core tables truly exist. Otherwise leave
    // schemaEnsured=false so the NEXT request retries (fixes D1-not-ready races
    // that previously cached a broken schema and made every write fail forever).
    try {
